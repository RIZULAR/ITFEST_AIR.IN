...
const SOIL_PARAMS = {
	...
	Berpasir: { fc: 0.15, wp: 0.05 },
	Lempung: { fc: 0.3, wp: 0.18 },
	Liat: { fc: 0.35, wp: 0.2 },
};
...
export function calculateDailyRiskScore(input) {
	...
	const daysSincePlanting = Math.max(
		0,
		Math.floor(
			(new Date(logDate || Date.now()) - new Date(plantDate || Date.now())) / (1000 * 60 * 60 * 24),
		),
	);
	const weekIndex = Math.min(11, Math.floor(daysSincePlanting / 7));

	const soil = SOIL_PARAMS[soilType] || SOIL_PARAMS.Latosol;
	const cropKey = cropType?.toLowerCase().includes('jagung') ? 'Jagung' : cropType?.toLowerCase().includes('padi') ? 'Padi' : 'Palawija';
	const crop = CROP_PARAMS[cropKey] || CROP_PARAMS.Padi;
	...
	const dateObj = new Date(logDate || Date.now());
	...
	const et0 =
		0.0027 * ra * (tempMean + 17.8) * Math.sqrt(Math.max(0, tempMax - tempMin));
	const etc = et0 * kc;
